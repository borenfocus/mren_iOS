...
- (NSURLSessionTask *)request:(RequestType)type
    ...
    NSString *urlStr = [URLUtil getUrlPath:URLString];
    //检查地址中是否有中文
...
-(void)uploadWithURL:(NSString *)url
    ...
    NSString *urlStr = [URLUtil getUrlPath:url];
    //检查地址中是否有中文
    urlStr = [NSURL URLWithString:urlStr]?urlStr:[self strUTF8Encoding:urlStr];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (code == successCode) {
            ...
            if (failure) {
                ...
                NSError *error = [NSError errorWithDomain:URL_HOST code:code userInfo:@{NSLocalizedDescriptionKey:message}];
                failure(error);
            }
